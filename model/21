...
def train_model(token):
    ...
    # For simplicity, we use the average price within each cluster as the forecast
    cluster_means = pd.Series(scaled_data.flatten()).groupby(clusters).mean()
    predicted_price = cluster_means.mean()
